...
const TIER_PRESETS = {
  Green: {
    ...
    TRI: 3.5,
    elevation: 850.0,
    distance_to_stream_m: 450.0,
    drainage_density: 1.2,
    ndvi_mean: 0.75,
    historical_event_count_500m: 0,
    rainfall_1h: 1.0,
    rainfall_3h: 3.0,
    rainfall_6h: 6.0,
    rainfall_24h: 12.0,
    rainfall_72h_antecedent: 18.0,
    rain_intensity_mm_hr: 1.5,
    antecedent_precipitation_index: 18.0,
    soil_saturation_ratio: 0.22,
    factor_of_safety: 3.2,
    factor_of_safety_min: 2.8,
    factor_of_safety_max: 3.6,
    land_use_class: 'Forest',
    gsi_susceptibility_class: 'Low',
    simulated_ffgs_signal: false,
    simulated_gsi_signal: false,
    iot_anomaly_flag: false,
  },
  Yellow: {
    slope_deg: 20.0,
    aspect: 160.0,
    ...
    TRI: 7.0,
    elevation: 750.0,
    distance_to_stream_m: 220.0,
    drainage_density: 1.6,
    ndvi_mean: 0.50,
    historical_event_count_500m: 1,
    rainfall_1h: 5.0,
    rainfall_3h: 14.0,
    rainfall_6h: 28.0,
    rainfall_24h: 48.0,
    rainfall_72h_antecedent: 68.0,
    rain_intensity_mm_hr: 6.5,
    antecedent_precipitation_index: 58.0,
    soil_saturation_ratio: 0.52,
    factor_of_safety: 1.45,
    factor_of_safety_min: 1.30,
    factor_of_safety_max: 1.65,
    land_use_class: 'Shrubland',
    gsi_susceptibility_class: 'Moderate',
    simulated_ffgs_signal: false,
    simulated_gsi_signal: false,
    iot_anomaly_flag: false,
  },
  Orange: {
    ...
    ndvi_mean: 0.35,
    historical_event_count_500m: 2,
    rainfall_1h: 10.0,
    rainfall_3h: 26.0,
    rainfall_6h: 52.0,
    rainfall_24h: 90.0,
    rainfall_72h_antecedent: 120.0,
    rain_intensity_mm_hr: 12.0,
    antecedent_precipitation_index: 92.0,
    soil_saturation_ratio: 0.74,
    factor_of_safety: 1.20,
    factor_of_safety_min: 1.05,
    factor_of_safety_max: 1.35,
    land_use_class: 'Cropland',
    gsi_susceptibility_class: 'High',
    simulated_ffgs_signal: true,
    simulated_gsi_signal: false,
    iot_anomaly_flag: false,
  },
  Red: {
    ...
    ndvi_mean: 0.12,
    historical_event_count_500m: 4,
    rainfall_1h: 26.0,
    rainfall_3h: 62.0,
    rainfall_6h: 115.0,
    rainfall_24h: 210.0,
    rainfall_72h_antecedent: 275.0,
    rain_intensity_mm_hr: 30.0,
    antecedent_precipitation_index: 170.0,
    soil_saturation_ratio: 0.94,
    factor_of_safety: 0.68,
    factor_of_safety_min: 0.50,
    factor_of_safety_max: 0.85,
    land_use_class: 'Bare',
    gsi_susceptibility_class: 'Very High',
    simulated_ffgs_signal: true,
    simulated_gsi_signal: true,
    iot_anomaly_flag: true,
  },
};

export default function ManualScenarioPanel({ onClose, externalPointRequest }) {
  // Initialize with a complete baseline so all fields are filled from the start
  const [values, setValues] = useState({ ...TIER_PRESETS.Yellow });
  ...
  const applyPreset = (tier) => {
    ...
    if (!preset) return;
    const pt = points.find((p) => p.hex_id === selectedPointId);
    const terrain = (pt && pt.has_real_static_features) ? pt.static_features : {};
    setValues({
      ...preset,
      ...terrain,
    });
    setPointNote(`Filled all fields with ${tier} scenario baseline${pt ? ` for ${pt.label}` : ''}.`);
  };
  ...
  const handlePointSelect = (hexId) => {
    ...
    // Pick appropriate baseline for the location (e.g. Idukki Pettimudi or Wayanad)
    const isHighHazard = pt.label.toLowerCase().includes('pettimudi') || pt.label.toLowerCase().includes('chooralmala');
    const baseline = isHighHazard ? TIER_PRESETS.Orange : TIER_PRESETS.Yellow;
    const prefill = { ...baseline };

    if (pt.has_real_static_features) {
      for (const k of STATIC_FIELD_KEYS) {
        if (pt.static_features[k] !== undefined && pt.static_features[k] !== null) {
          prefill[k] = pt.static_features[k];
        }
      }
    }
    setValues(prefill);
    setPointNote(`Pre-filled all fields for ${pt.label} (real terrain data + active meteorological conditions).`);
  };

  // Clicking a real pin/hex on the map while this panel is open loads that
  // point's data the same way picking it from the dropdown does.
  useEffect(() => {
