...
def load_rainfall_lookup() -> dict[str, dict[str, float]]:
    ...
    Falls back gracefully if file doesn't exist (Phase 1 not yet run) with a
    loud warning — per CLAUDE.md, we do not silently substitute fake data.
    """
    if not RAINFALL_JSON.exists():
        print(
            f"WARNING: {RAINFALL_JSON} not found. "
            "Run data/scripts/ingest_rainfall.py (Phase 1) first.\n"
            "Rainfall features will be None/NaN in the sample set — "
            "re-run after Phase 1 to get real values.",
            file=sys.stderr,
        )
        return {}
